...
Fixture-runnable operations copilot for X's AI Note Writer API. The local build runs with no API keys, no live network calls, and no committed secrets. It implements the intake, evidence, drafting, critique, X evaluation, operator approval, submission gate, feedback, admission, writing-limit, cost, and operational eval flows described in `CommunityNotes1.md` and corrected by `CommunityNotes2.md`.
