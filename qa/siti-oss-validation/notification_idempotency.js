...
AWS.SNS.prototype.publish = function(payload, callback) {
  publishCalls.push({ payload: payload, at: Date.now() });
  setImmediate(function() {
    callback(null, { MessageId: 'audit-' + publishCalls.length });
  });
};
...
async function main() {
  ...
  await notify.send(copyParams());
  await notify.send(copyParams());
  const sequential = publishCalls.map(function(item, index) {
    return { index: index + 1, topic: item.payload.TopicArn, message: item.payload.Message };
  });

  publishCalls.length = 0;
  const concurrentAttempts = 50;
  const sends = Array.from({ length: concurrentAttempts }, function() {
    return notify.send(copyParams());
  });
  const concurrentResults = await Promise.allSettled(sends);
  const concurrent = {
    attempts: concurrentAttempts,
    fulfilled: concurrentResults.filter(function(item) { return item.status === 'fulfilled'; }).length,
    rejected: concurrentResults.filter(function(item) { return item.status === 'rejected'; }).length,
    publish_call_count: publishCalls.length,
    unique_payload_count: new Set(publishCalls.map(function(item) { return JSON.stringify(item.payload); })).size,
  };
  ...
  function invokeHandler() {
    return new Promise(function(resolve) {
      handler(event, {}, function(_error, response) { resolve(response); });
    });
  }
